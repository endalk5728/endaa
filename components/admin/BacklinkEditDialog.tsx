...
import { Backlink, BacklinkFormData } from '@/types/post';
...
interface BacklinkEditDialogProps {
  backlink: Backlink;
  onComplete: () => void;
}

export default function BacklinkEditDialog({ backlink, onComplete }: BacklinkEditDialogProps) {
  ...
  const form = useForm<BacklinkFormData>({
    ...
    defaultValues: {
      ...
      rel_attribute: backlink.rel_attribute,
      is_active: backlink.is_active,
    },
  });
  ...
  async function onSubmit(values: BacklinkFormData) {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/backlinks/${backlink.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        throw new Error('Failed to update backlink');
      }

      toast({
        title: 'Backlink updated',
        description: 'Your backlink has been successfully updated.',
      });
      setIsOpen(false);
      onComplete();
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to update backlink. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button type="submit" onClick={form.handleSubmit(onSubmit)} disabled={isLoading}>
            {isLoading ? 'Updating...' : 'Update Backlink'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
